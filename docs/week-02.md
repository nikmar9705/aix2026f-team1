...
```
[Instructions]
Add a memo search feature. It searches keywords in title and body.
...
[Completion Criteria]
- Called via GET /memos/search?q=keyword
- Returns only memos where keyword appears in title OR body
- Returns only the user's own memos
- If q is empty, return 400 and { ok: false, error }
```
...
| | → 무엇을 어겼는가 | Express 라우터 구조 미사용, REST API 엔드포인트(`GET /memos/search`) 미구현, JS 대신 Python 언어로 작성됨 |
| ④ | 사람이 직접 고친 지점 | **전면 수정 필요 (3곳 이상)** |
| ⑤ | **본인 메모만 반환되는가** | **아니오** |
...
```
A조가 만든 것은 "틀린 코드"가 아니라 다른 프로젝트의 코드였다.

아이만의 결과는 Node.js로 나왔다. 하지만 기존 프로젝트에 함수를 추가한 것이
아니라, src/ 폴더와 data/memos.json 파일을 가진 새 프로젝트를 처음부터 만들었다.
아킬의 결과는 아예 Python으로 나왔고, MemoManager 클래스와 add_memo,
search_memos 처럼 이 프로젝트에 없는 구조를 만들었다.

두 사람은 같은 프롬프트를 썼지만 언어부터 달랐다. 그런데 틀린 자리는 같았다.
둘 다 본문 컬럼을 content 로 추측했고 (실제는 body), 둘 다 user_id 가 없었다.

B조는 지어낸 이름이 거의 없었고, 기존 service.js 와 같은 방식으로 작성했고,
쿼리에 user_id 조건이 들어 있었다.

가장 중요한 차이는 ⑤번이다. A조는 두 명 다 "아니오", B조는 "예"였다.
A조 코드는 에러 없이 잘 돌아간다. 그런데 남의 메모까지 검색된다.
```

**4-2. A조의 실패는 모델 탓입니까, 우리가 주지 않은 탓입니까? 근거를 들어 적으세요.**

```
대부분 우리가 주지 않은 탓이다. A조의 실패를 두 가지로 나눌 수 있다.

첫째, 자료가 없으면 알 수 없는 것이다.
- 컬럼 이름이 content 가 아니라 body 인 것 → schema.sql 을 못 봤다
- 프로젝트가 JS 인 것 → 파일을 하나도 주지 않았다
- 응답 형식이 { ok, data } 인 것 → CONVENTIONS.md 를 못 봤다
이것들은 모델이 잘못 추측한 게 아니다. 추측할 근거가 아예 없었다.

둘째, 지시에 없었던 것이다.
A조 프롬프트에는 "본인 메모만"이라는 요구가 없었다. B조 종료조건에는 있었다.
이것은 컨텍스트 부족이 아니라 명세 누락이다.

증거가 하나 더 있다. 아이만의 에이전트는 시작할 때
"프로젝트 디렉토리가 비어있어서 처음부터 만들어야 합니다" 라고 말했다.
모를 때 모른다고 말한 것이다.

반대로 GET /memos/search?q= 형태는 자료 없이도 맞췄다.
즉 관례로 알 수 있는 것은 맞히고, 이 프로젝트에만 있는 것은 다 틀렸다.
이것이 모델 능력의 문제가 아니라는 증거다.

다만 모델 책임도 조금은 있다. A조에서는 언어나 스키마를 되묻지 않고 바로
추측해서 작성했다. B조에서는 대소문자 구분 여부를 먼저 질문했다.
자료가 있을 때만 질문했다는 점은 기록해 둘 만하다.
```

**4-3. B조가 준 자료 중 결과를 가장 크게 바꾼 것 하나를 꼽는다면 무엇입니까? 왜 그렇게 생각합니까?**

```
schema.sql 이다. 하지만 두 파일은 서로 다른 일을 했고, 하나만으로는 부족했다.

A조의 실패를 자료별로 나누면 이렇게 갈라진다.
- schema.sql 이 없어서 → content, memos.json, MemoManager 같은 이름을 지어냈다
- CONVENTIONS.md 가 없어서 → user_id 누락, 응답 형식 위반, 명명 규칙 위반

CONVENTIONS.md 는 "모든 쿼리에 user_id 조건을 포함한다"고 요구한다. 하지만
그 컬럼 이름이 user_id 인지 owner 인지 author_id 인지는 알려주지 않는다.
schema.sql 은 이름을 알려준다. 하지만 왜 그것으로 걸러야 하는지는 말하지 않는다.

schema.sql 을 고른 이유는, 이름을 모르면 규칙을 알아도 코드를 쓸 수 없기 때문이다.
규약은 지키려는 의도를 만들고, 스키마는 그것을 실행 가능하게 만든다.

반대 의견도 적어 둔다. 컬럼 이름이 틀리면 에러가 나서 사람이 바로 잡는다.
하지만 user_id 가 빠진 코드는 에러 없이 돌아간다. 아무도 모른다.
"안 돌아가는 코드"보다 "돌아가지만 위험한 코드"가 더 무섭다고 보면
CONVENTIONS.md 를 고를 수도 있다.
```
