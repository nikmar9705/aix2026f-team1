...
| 질문 | 후보 A | 후보 B | 후보 C |
|---|---|---|---|
| 이 안에 **누구의 정보**가 들어가는가<br>그 사람이 우리 사용자와 같은 사람인가 | 포스터를 만들고 올리는 사람(동아리 임원, 공모전 공지를 올리는 학과 조교, 학생회)의 정보 — 포스터에 적힌 이름, 전화번호, 이메일, 카톡 오픈채팅 링크. 우리 사용자(외국인 유학생)와 다른 사람이다 |학생이 작성한 식당/음식 리뷰 및 평가. 리뷰 작성자 = 사용자와 같은 사람 (서로 정보를 공유하는 무슬림 학생들)  | Only the user's grocery information, such as the food they bought, purchase date, and expiration date. |
| 쓰고 싶어도 **못 쓰는 사람**은 누구인가 | 영어도 편하지 않은 유학생 | none | none |
...
| 3 | A | 번역기로 포스터를 찍어도 참가 자격·마감일·신청 방법을 정확히 알기 어렵다직접 해보기 | 직접 해보기 | 실제 교내 포스터 10장을 파파고·구글 렌즈로 번역해, 세 정보가 모두 맞게 나온 장 수를 센다 |  |  |
